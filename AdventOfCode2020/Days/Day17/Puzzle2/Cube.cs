...
namespace AdventOfCode2020.Days.Day17.Puzzle2
{
    ...
    {
        private SortedDictionary<long, SortedDictionary<long, SortedDictionary<long, SortedDictionary<long, CellState>>>> cells;
        ...
        public Cube()
        {
            cells = new SortedDictionary<long, SortedDictionary<long, SortedDictionary<long, SortedDictionary<long, CellState>>>>();
        }
        ...
        public void SetPlace(CellState value, params long[] dims)
        {
            ...
            if (cells.ContainsKey(y))
            {
                if (cells[y].ContainsKey(x))
                {
                    ...
                    {
                        UpdateRanges(y, x, z, w);
                        cells[y][x][z][w] = value;
                        return;
                    }
                    else
                    {
                        cells[y][x][z] = new SortedDictionary<long, CellState>();
                    }
                }
                else
                {
                    cells[y][x] = new SortedDictionary<long, SortedDictionary<long, CellState>>();
                }
            }
            else
            {
                cells[y] = new SortedDictionary<long, SortedDictionary<long, SortedDictionary<long, CellState>>>();
            }
            SetPlace(value, y, x, z, w);
        }

        private void UpdateRanges(long y, long x, long z, long w)
        {
            bounds[0] = UpdateRange(y, bounds[0]);
            bounds[1] = UpdateRange(x, bounds[1]);
            bounds[2] = UpdateRange(z, bounds[2]);
            bounds[2] = UpdateRange(w, bounds[3]);
        }
        ...
        public CellState? GetValue(params long[] dims)
        {
            ...
            if (cells.ContainsKey(y))
            {
                if (cells[y].ContainsKey(x))
                {
                    if (cells[y][x].ContainsKey(z))
                    {
                        if (cells[y][x][z].ContainsKey(w))
                        {
                            return cells[y][x][z][w];
                        }
                        else
                        {
        ...
        public long GetNoOfActiveCube()
        {
            ...
            foreach (var dim1 in cells)
            {
                foreach (var dim2 in dim1.Value)
                {
                    ...
                    {
                        foreach (var dim4 in dim3.Value)
                        {
                            if (dim4.Value == CellState.Active)
                            {
                                result++;
                            }
                        }
